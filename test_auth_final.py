...
def test_auth_flow():
    ...
    print(f"Test telephone: {test_user['telephone']}")
    ...
    try:
        ...
        if resp.status_code == 201:
            ...
            print(f"   User ID: {signup_data['id']}")
        else:
    ...
    # 2. Test Login WITH telephone
    print("\n2. Testing Login (with telephone)...")
    try:
        login_data = {
        ...
        if resp.status_code == 200:
            ...
            # FIX: Use 'token' field instead of 'access_token'
            token = response_data.get('token')
            if token:
                print("   ✅ Login Successful!")
                print(f"   Token: {token[:50]}...")
                print(f"   Full response: {json.dumps(response_data, indent=4)}")
                return token, test_user["email"]
            else:
...
def test_chat_with_token(token, email):
    if not token:
        print("\n❌ No token - skipping chat tests")
        return
    
    print(f"\n3. Testing Chat Endpoints for {email}...")
        ...
        try:
            ...
            if resp.status_code == 201:
                ...
                if data.get('helplines'):
                    print(f"   📞 Helplines: {data['helplines']}")
            else:
                print(f"   Response: {resp.text}")
        except Exception as e:
...
if __name__ == "__main__":
    token, email = test_auth_flow()
    test_chat_with_token(token, email)
    print("\n🎉 Testing Complete!")
